...
public class LoadingView extends FrameLayout {
	...
	private ImageView indicationIm;// 底部阴影图片

	private TextView loadTextView;// 底部文字
	...
	protected void onFinishInflate() {
		...
		mDistance = Utility.dip2px(getContext(), 54f);
	...
	public void setLoadingText(CharSequence loadingText) {
		if (loadingText != null)
			loadText = loadingText.toString();
		loadTextView.setText(loadingText);
		System.out.println(loadingText);
	}

	public void setLoadingTexts(List<String> loadingTexts) {
		this.loadTexts = loadingTexts;
		if (loadingTexts != null && loadingTexts.size() > 0) {
			setLoadingText(loadingTexts.get(0));
		}
	}
			...
			public void onAnimationEnd(Animator animation) {
				if (loadTexts != null && loadTexts.size() > 0) {
					if (!TextUtils.isEmpty(loadText)
							&& loadTexts.contains(loadText)) {
						int index = loadTexts.indexOf(loadText);
						if (!(index < 0)) {
							setLoadingText(loadTexts.get(index == (loadTexts
									.size() - 1) ? 0 : (index + 1)));
						}
					}
				}
